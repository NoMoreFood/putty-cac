...
static WeierstrassPoint *parse_x509_ecdsa_pubkey(
    ptrlen cert, const char *keyalg, const struct ec_curve *curve)
{
    LPCBYTE spk;
    size_t spk_len;
    if (!cert_x509_ecdsa_public_key(
            keyalg, cert.ptr, cert.len, &spk, &spk_len))
        return NULL;

    size_t field_len = (curve->fieldBits + 7) / 8;
    if (spk_len == 0 ||
        !((spk[0] == 2 || spk[0] == 3) && spk_len == 1 + field_len) &&
        !(spk[0] == 4 && spk_len == 1 + 2 * field_len))
        return NULL;
    return ecdsa_decode(make_ptrlen(spk, spk_len), curve);
}

static void x509_ecdsa_freekey(ssh_key *key);

static ssh_key *x509_ecdsa_new_priv(
...
static ssh_key *x509_ecdsa_new_priv_openssh(
...
static bool x509_ecdsa_has_private(ssh_key *key)
{
    (void)key;
    return false;
}
...
static ssh_key *x509_ecdsa_new_pub(const ssh_keyalg *self, ptrlen pub)
{
    ...
    xkey->ecdsa.publicKey = parse_x509_ecdsa_pubkey(
        make_ptrlen(cert, cert_len), self->ssh_id, xkey->ecdsa.curve);
...
static bool x509_ecdsa_verify(ssh_key *key, ptrlen sig, ptrlen data)
{
    ...
    if (ecc_weierstrass_is_identity(u1Gu2H)) {
...
const ssh_keyalg ssh_x509v3_ecdsa_nistp256 = {
    ...
    .new_priv = x509_ecdsa_new_priv,
    .new_priv_openssh = x509_ecdsa_new_priv_openssh,
