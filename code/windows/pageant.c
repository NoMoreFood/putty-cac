    ...
    struct pageant_pubkey *key)
{
    ...
    switch (key->ssh_version) {
      ...
      case 2: {
        ...
        {
            // trim sk- algorithms since they are very long
            char sAlg[32], sHash[64];
            if (sscanf(listentry->s, "sk-%[^@]@openssh.com%s\t", sAlg, sHash) == 2)
            {
                strbuf_clear(listentry);
                put_fmt(listentry, "%s\t%s", sAlg, sHash);
            }

            // append the type
            put_fmt(listentry, "\t%.4s", key->comment);

            // append the string descriptor
            LPSTR subjectname = cert_subject_string(key->comment);
			put_fmt(listentry, "\t%s", subjectname);
			sfree(subjectname);
        } else
#endif // PUTTY_CAC
